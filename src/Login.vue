  ...
  <n-flex justify="center">
    <n-flex vertical justify="center" style="height: calc(100vh - 2.2rem)">
      <n-alert type="success" :bordered="false" :show-icon="false"
        >Use your Telegram phone number to login. Password is not
        supported.</n-alert
      >
      <n-input-group>
        <n-input
        ...
        />
        <n-button @click="sendCode" :loading="disableSendCode"
          >Send Code</n-button
        >
      </n-input-group>
      <n-input-group>
        <n-input v-model:value="phoneCode" type="text" placeholder="Code" />
        <n-button @click="login" type="primary" :disabled="!phoneCode"
          >Login</n-button
        >
      </n-input-group>
    </n-flex>
...
async function sendCode() {
  try {
    ...
    message.info("Check your other telegram app for the phone code.", {
...
async function login() {
  try {
    await client.signInUser(authCredentials, {
